...
export class ChunkReceiver {
  ...
  constructor(firstChunk, bufferSize) {
    // Create factory function object
    this.util = Util();

    if (!firstChunk || !firstChunk instanceof Message.Request) {
  ...
  processChunk = function(chunk) {
    ...
    switch (chunk.continuationFlag) {
      case Message.flags.continued:
        break;
      case Message.flags.end:
        this.totalBytes = chunk.byteRange.start + chunkSize - 1;
        break;
      case Message.flags.abort:
        this.abort();
    ...
    if (chunk.byteRange.start === nextStart) {
      ...
      while (!this.util.isEmpty(this.incontiguousChunks)) {
