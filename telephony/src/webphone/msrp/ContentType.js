export async function ContentType(msrp) {
  /**
  ...
  ContentType.prototype.parseSdpTypeSelector = function(selector) {
    ...
    while (selector.charAt(endIndex) === ';') {
      ...
      this.params[param] = msrp.Util.decodeSdpFileName(value);
    }
  };
  ...
  ContentType.prototype.toSdpTypeSelector = function() {
    ...
    for (param in this.params) {
      selector = selector.concat(';', param, '="',
        msrp.Util.encodeSdpFileName(this.params[param]), '"');
    }
  ...
  ContentType.prototype.parseContentTypeHeader = function(header) {
    ...
    while (header.charAt(endIndex) === ';') {
      ...
      this.params[param] = msrp.Util.decodeQuotedString(value);
    }
  };
  ...
  ContentType.prototype.toContentTypeHeader = function() {
    ...
    for (param in this.params) {
      header = header.concat(';', param, '="',
        msrp.Util.encodeQuotedString(this.params[param]), '"');
    }
