import {IncomingResponse, Response, IncomingRequest} from './Message.js';
  ...
  return {  
    ...
    parseMessage(data) {
      ...
      if (data instanceof ArrayBuffer) {
        ...
        console.info('Unexpected parameter type');
        return null;
      }
      ...
      if (tokens[2].length === 3 && (statusCode = parseInt(tokens[2], 10))) {
        if (tokens.length > 3) {
          ...
          msgObj = new IncomingResponse(tokens[1], statusCode, comment);
        } else {
          msgObj = new IncomingResponse(tokens[1], statusCode);
        }
      } else if (tokens.length === 3) {
        msgObj = new IncomingRequest(tokens[1], tokens[2]);
      } else {
        console.warn('Error parsing message: unexpected first line format: ' + firstLine);
        return null;
      }
